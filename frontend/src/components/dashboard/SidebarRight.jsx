  ...
  return (
    <div className="h-full flex flex-col p-4">
      ...
      <div className="flex-1 bg-[#fdf6e3] rounded-sm shadow-lg p-4 border-t-8 border-blue-400 relative overflow-hidden">
        ...
        <h3 className="text-black font-bold mb-4 mt-2 border-b-2 border-dashed border-gray-300 pb-2 italic" style={{ fontFamily: 'Inkfree', fontSize: '1.4rem' }}>
          Documentos Generados
        </h3>

        <div className="space-y-4 overflow-y-auto max-h-[50vh] pr-2 custom-scrollbar">
          {stats.archivos.length === 0 ? (
            <p className="text-black text-sm italic">No hay archivos todavía...</p>
          ) : (
            stats.archivos.map((doc) => (
              <div key={doc.id} className="flex flex-col border-b border-gray-200 pb-2 group">
                <span className="text-sm font-medium text-black truncate">{doc.nombre}</span>
                <div className="flex justify-between items-center mt-1">
                  <span className="text-[10px] text-black uppercase tracking-widest">{doc.fecha}</span>
                  <a 

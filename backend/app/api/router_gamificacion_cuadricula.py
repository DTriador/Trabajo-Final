...
def _dibujar_encabezado_pdf(c, width, height, nombre_escuela, nombre_materia, division, fecha):
    """
    Dibuja el encabezado institucional.
    El campo 'Nombre del Alumno/a' queda en blanco para que lo complete el alumno.
    ...
    c.setFont("Helvetica", 11)
    c.drawString(1.5*cm + 5.6*cm, y, "_______________________________")
    y -= 0.7*cm
    # Línea separadora
    c.setStrokeGray(0.6)
    c.setLineWidth(0.8)
...
async def generar_crucigrama(
    ...
    try:
        ...
        - "pista": una sola oración corta tipo definición de diccionario.
        - Sin repetidos.
        """
...
def _renderizar_pdf_crucigrama(tema, grilla, horizontales, verticales,
                                nombre_escuela, nombre_materia, division, fecha):
    buf = io.BytesIO()
    c = pdfcanvas.Canvas(buf, pagesize=A4)
    width, height = A4

    y_after_header = _dibujar_encabezado_pdf(
        c, width, height, nombre_escuela, nombre_materia, division, fecha
    )
...
async def generar_unir_flechas(
    ...
    try:
        ...
        - "definicion": una oración corta y clara, máximo 130 caracteres.
        - Sin repetidos.
        """
...
def _renderizar_pdf_unir_flechas(tema, pares, nombre_escuela, nombre_materia, division, fecha):
    buf = io.BytesIO()
    c = pdfcanvas.Canvas(buf, pagesize=A4)
    width, height = A4
